...
const Home = () => {
  ...
  useEffect(() => {
    const fetchNewReleases = async () => {
      try {
        ...
        const clientId = '829c5514fe3244749d39e30ba2d23b75';
        const clientSecret = 'd01f5732d95547b58cbf9c27868d931e';
  ...
  // Effect to handle left and right arrow key events
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === 'ArrowLeft' && currentReleaseIndex > 0) {
        setCurrentReleaseIndex(currentReleaseIndex - 1);
        setShowMessage(false); // Hide the message when an arrow key is pressed
      } else if (event.key === 'ArrowRight' && currentReleaseIndex < newReleases.length - 1) {
        setCurrentReleaseIndex(currentReleaseIndex + 1);
        setShowMessage(false); // Hide the message when an arrow key is pressed
      }
    };
